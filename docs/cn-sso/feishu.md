...
本页讲飞书登录方向的落地对接；面向企业客户的 OpenAPI、服务端 SDK、事件、权限与交付评价见 [飞书标准安全评价](./feishu-review.md)。企业微信对接见 [企业微信](./wecom.md)。
...
| 支持协议 | 管理后台提供的企业 SSO 以 SAML 配置为主，具体以租户版本为准 | 开放平台网页授权登录为 OAuth2 式授权码流程；其它应用 SSO 形态按应用类型确认 |
...
飞书的企业 OpenAPI、SDK、事件与权限评价见 [飞书标准安全评价](./feishu-review.md)；SAML、OAuth2/OIDC 基础见本站对应专题。
