...
国内的飞书、钉钉、微信、企业微信,以及喜马拉雅、QQ 音乐这类 SDK 平台**都能做登录 / SSO**,但它们和标准协议(SAML / OAuth 2.0 / OIDC)的贴合程度差别很大,踩坑点也和对接 Okta / Entra ID 完全不同。

本区每个平台都拆成**两篇**:一篇讲**怎么落地对接**,一篇**逐项评价它与标准的差距、风险与改造建议**。想快速接入看前者,想评估风险 / 推动对方改造看后者。**各平台的细节、对照表、参考来源都在其各自页面**,本页只做索引与导航。
...
| 方向 | 平台扮演的角色 | 典型诉求 |
|------|--------------|---------|
| **A. 用企业已有账号登录协作平台** | 平台 = **SP**(服务提供方) | 企业已有 IdP(AD / Okta / IDaaS),想让员工用统一账号登进平台 |
| **B. 用平台账号登录其他系统** | 平台 = **IdP** / 身份源 | 拿平台当登录入口,扫码登进 OA、云桌面等 |

各平台分别落在哪个方向、支持什么协议,见其对接页;微信 / 企业微信 / 喜马拉雅 / QQ 音乐属于方向 B(平台作身份源)。
...
各平台的官方文档链接见其对接实现页与评价页末尾的「参考来源 / 参考标准」。
