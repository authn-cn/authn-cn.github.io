...
本页讲**怎么落地对接** QQ 音乐开放平台(车机 / 设备端 SDK)的「第三方登录 / 账号绑定」——用你自己的账号体系绑定 QQ 音乐账号,实现同登同出并访问 QQ 音乐资源。它与标准 OIDC/OAuth2 的差距、风险与改造建议单独成文:[QQ 音乐第三方登录:与标准的差距](./qqmusic-review.md)。
...
- [QQ 音乐第三方登录:与 OIDC/OAuth2 标准的差距与改造建议](./qqmusic-review.md)
