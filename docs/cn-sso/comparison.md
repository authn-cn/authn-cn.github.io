...
| 14 | [QQ 互联](./qq-connect-review.md) | ToC | **41.0** | D | code/state 与刷新轮换可用；Implicit、查询串凭据和私有响应遗留明显 |
| 15 | [微博开放平台](./weibo-review.md) | ToC | **40.0** | D | OAuth 式授权可接入；OIDC、PKCE 和标准 token 治理证据不足 |
| 16 | [百度账号](./baidu-review.md) | ToC | **37.5** | E | 查询串凭据、可选 state、宽松回调和超长 refresh token 不适合新基线 |
| 17 | [QQ 音乐](./qqmusic-review.md) | 内容/设备 | **36.5** | E | SDK 绑定可工作；无标准身份协议且采用自定义 MD5 回调 |
...
当前 17 个系统没有一家在全部八个维度达到 A。短期可用统一身份网关控制私有适配风险；长期目标不应是永久维护 17 个适配器，而应推动平台只在 OIDC、OAuth、SAML、SCIM 等标准端点增加新能力，让通用实现可以替换厂商 SDK。
