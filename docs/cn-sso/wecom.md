...
"用企业微信登录"让企业员工用企业微信扫码登进内部系统(OA、云桌面、后台等)。它采用 **OAuth2 授权码式的私有流程**:PC 网页上展示一个二维码,员工用手机企业微信 App 扫码并确认,浏览器拿到一个一次性的 `code`,再由**后端**用 `code` 换取该员工的身份。
...
> 本页讲登录落地；企业微信服务端 API、回调、权限、第三方应用与 SDK 的统一评价见 [企业微信标准安全评价](./wecom-review.md)。
